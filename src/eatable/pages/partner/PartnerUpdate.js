    ...
    for (const fileData of post.fileList) {
      if (fileData) {
        if (fileData.imageUrl.startsWith('data:image')) {
          ...
          const file = new File([blob], 'image.png', { type: 'image/png' }, fileData.id);
            ...
            <div className={"gallery--box " + (post.fileList.length > 0 && "true")} {...getRootProps()}>
              ...
              {post.fileList.length === 0 && (
                <div className="no--case">
                  <span className="text">Drop files here or click to upload.</span>
                  <br />
                </div>
              )}
            </div>
            {post.fileList.map((image, idx) => (
              <div key={idx} className="gallery--list" draggable="true" onDragStart={(e) => handleDragStart(e, idx)} onDrop={(e) => handleDrop(e, idx, image.id)} onDragOver={handleDragOver}>

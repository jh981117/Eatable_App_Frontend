...
import { TollTwoTone } from "@material-ui/icons";
...
const ApplyList = () => {
  ...
  const [disable, setDisable] = useState(
    JSON.parse(localStorage.getItem("disableState")) || []
  );
  const [page, setPage] = useState(0); // 현재 페이지
  const [totalPages, setTotalPages] = useState(0); // 전체 페이지 수
  const [filtered, setFiltered] = useState([]); // 필터링된 리스트
    ...
    fetch(`http://localhost:8080/api/req/totalListPage?page=${page}`)
      ...
      .then((data) => {
        const filteredData = data.content.filter((item) =>
          ["OPEN", "OPEN_READY", "WAIT"].includes(item.partnerReqState)
        );
        ...
        <Row>
          <Col>
            <Table striped bordered hover size="sm" className="list_table">
              ...
              <tbody>
                {filteredLists
                  .slice(page * number, (page + 1) * number)
                  .map((list) => (
                    <tr key={list.id}>
                      <td>{list.id}</td>
